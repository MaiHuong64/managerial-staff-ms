...
export const getHoSoBoNhiemById = async (client: any, id: number) => {
    const result = await client.query(
        ...
        hsbn.chi_tiet_pa_id, hsbn.phieu_chu_truong_id,
        ...
        LEFT JOIN vien_chuc vc ON vc.id = COALESCE(ctbn.vien_chuc_id, (SELECT vien_chuc_id FROM phieu_chu_truong WHERE id = hsbn.phieu_chu_truong_id))
        LEFT JOIN chi_tiet_dot_bo_nhiem ctdbn ON ctdbn.id = ctbn.chi_tiet_dot_bo_nhiem_id
        LEFT JOIN phieu_chu_truong pct ON pct.id = COALESCE(ctdbn.phieu_chu_truong_id, hsbn.phieu_chu_truong_id)
...
export const insertHoSoBoNhiem = async (client: any, maHoSo: string, chiTietPA: number | null, phieuChuTruongId: number | null, ghiChu: string | null) => {
    const result = await client.query(
        `INSERT INTO ho_so_bo_nhiem (ma_ho_so, ngay_lap, chi_tiet_pa_id, phieu_chu_truong_id, ghi_chu) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
        [maHoSo, new Date(), chiTietPA, phieuChuTruongId, ghiChu]
    );
...
export const checkHoSoExistsByPhieuChuTruongId = async (client: any, id: number) => {
    const result = await client.query(`
        SELECT phieu_chu_truong_id FROM ho_so_bo_nhiem WHERE phieu_chu_truong_id = $1
    `, [id]);
    return result.rowCount > 0
}
export const getHoSoByPhieuChuTruongId = async (id: number) => {
    const result = await pool.query(`
        SELECT hsbn.id, hsbn.ma_ho_so, hsbn.ngay_lap, hsbn.trang_thai, hsbn.ghi_chu,
        hsbn.chi_tiet_pa_id, hsbn.phieu_chu_truong_id,
        vc.ho_va_ten, vc.ma_vien_chuc, cd.ten_chuc_danh, dv.ten_don_vi,
        COUNT(cths.id)::int AS so_tai_lieu
        FROM ho_so_bo_nhiem hsbn
        JOIN phieu_chu_truong pct ON hsbn.phieu_chu_truong_id = pct.id
        LEFT JOIN vien_chuc vc ON vc.id = pct.vien_chuc_id
        LEFT JOIN chuc_danh_quan_ly cd ON cd.id = pct.chuc_danh_id
        LEFT JOIN don_vi dv ON dv.id = vc.don_vi_id
        LEFT JOIN chi_tiet_ho_so cths ON cths.ho_so_bn_id = hsbn.id
        WHERE hsbn.phieu_chu_truong_id = $1
        GROUP BY hsbn.id, vc.ho_va_ten, vc.ma_vien_chuc, cd.ten_chuc_danh, dv.ten_don_vi
    `, [id]);
    return mapArrayToCamel(result.rows);
}
