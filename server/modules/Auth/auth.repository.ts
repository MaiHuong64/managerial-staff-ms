...
export const findByUsername = async (username: string) => {
    const query = `
        SELECT t.id, t.ten_dang_nhap, t.mat_khau, t.vai_tro, t.vien_chuc_id,
...
export const createUser = async (username: string, hashedPassword: string, role: string, vienChucId: number) => {
    const result = await pool.query(
