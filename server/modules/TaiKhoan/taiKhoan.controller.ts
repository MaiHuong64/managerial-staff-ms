...
import * as TaiKhoanService from "./taiKhoan.service";
...
export const updateVaiTro = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Cập nhật vai trò thành công", data: result});
    } catch (error: any) {
        return res.status(500).json({ success: false, message: error.message || "Lỗi khi cập nhật vai trò" });
    }
};

export const getByVienChucId = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ success: false, message: error.message || "Lỗi khi lấy thông tin tài khoản" });
    }
};

export const getDanhSachTK = async (req: Request, res: Response) => {
    try {
...
export const updateTrangThai = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        return res.status(200).json({
            success: true,
            message: trangThai === 1 ? "Mở khoá tài khoản thành công" : "Khoá tài khoản thành công",
            data: result,
        });
    } catch (error: any) {
